...
const { sendCancellationEmail, sendWelcomeEmail } = require("../utils/email");
...
router.get("/users/login", async (req, res) => {
  try {
    ...
    res.status(404).send({ error: "user not found with that email" });
  }
});
...
const upload = multer({
  ...
  fileFilter(req, file, callback) {
    if (!file.originalname.match(/.(jpg|png|jpeg)$/gm)) {
      return callback(new Error("You can upload only jpg, png, jpeg"));
    }
